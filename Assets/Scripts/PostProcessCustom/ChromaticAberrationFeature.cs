...
public class ChromaticAberrationFeature : ScriptableRendererFeature
{
  ...
  {
    chromaticAberrationRenderPass = new ChromaticAberrationRenderPass(
      "My custom pass",
  ...
  public override void AddRenderPasses(ScriptableRenderer renderer, ref RenderingData renderingData)
  {
    ...
    chromaticAberrationRenderPass.Setup(cameraColorTargetIdent);

    // Ask the renderer to add our pass.
    // Could queue up multiple passes and/or pick passes to use
    renderer.EnqueuePass(chromaticAberrationRenderPass);
  }
}
